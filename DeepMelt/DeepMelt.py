...
seed = 162
#We get 10 results by executing the code once with 10 different random number seeds,
# and Figure 14 and Figure 15 describe the mean and standard deviation of the 10 experiments.
...
class model(torch.nn.Module):
    ...
    def __init__(self, input_size, hidden_size, num_layers, nb_channels_raman, p_drop=0.5,
        ...
        """Initialization of DeepMelt model
    ...
    def senond_grade_compensation(self, x, T):
        ag = self.ag(x, T)
        myega = self.myega(x, T)
        am = self.am(x, T)
        cg = self.cg(x, T)
        tvf = self.tvf(x, T)
        c21 = torch.cat([ag, myega, am, cg, tvf], 1)
        c22 = torch.cat([ag, myega, am, cg, tvf], 1)
        for layer in self.machine_compensation_self_attention:  # Feedforward
            # c2 = layer(c2)
            c21 = self.dropout(self.activation_function(layer(c21)))
        c21 = self.machine_compensation_self_attention_output(c21)


        # c22 =torch.mul(c21.unsqueeze(2), F.softmax(c22).unsqueeze(1))
        # c22 = torch.matmul(c21.unsqueeze(1), F.softmax(c22).unsqueeze(2))
        c22 = torch.matmul(c21.unsqueeze(1), F.softmax(c22).unsqueeze(2))
    ...
    def self_attention(self, x, T):
        ...
        # c22 = torch.matmul(c21.unsqueeze(1), F.softmax(c22).unsqueeze(2))
        # c22 = c22.squeeze(2)
        a = F.softmax(c22).unsqueeze(2)
        return a

    def self_attention1(self, x, T):
        return self.self_attention(x,T)[:,0]

    def self_attention2(self, x, T):
        return self.self_attention(x,T)[:,1]

    def self_attention3(self, x, T):
        return self.self_attention(x,T)[:,2]

    def self_attention4(self, x, T):
        return self.self_attention(x,T)[:,3]

    def self_attention5(self, x, T):
        return self.self_attention(x,T)[:,4]
    ...
    def fuse(self, x, T):
        ...
        return torch.add(v, self.lambda5 * torch.tanh(fix_v)), torch.abs(self.lambda5 *torch.tanh(fix_v))
...
def training(neuralmodel, ds, criterion, optimizer, save_switch=True, save_name="./temp", train_patience=50,
    ...
    while val_ex <= train_patience:
        ...
        y_compensation_train_error =neuralmodel.visoscity_fix_error_mean(ds.x_visco_train, ds.T_visco_train)
        ...
        y_compensation_valid_error =neuralmodel.visoscity_fix_error_mean(ds.x_visco_valid, ds.T_visco_valid)
        ...
        if mode == "pretrain":
            ...
            loss = (loss_ag + loss_myega + loss_am + loss_cg + loss_tvf  + loss_entro) +  loss_compensation +(loss_compensation_error_min)
        ...
        with torch.set_grad_enabled(False):
            ...
            if mode == "pretrain":
                ...
                loss_v = (loss_ag_v + loss_myega_v + loss_am_v + loss_cg_v + loss_tvf_v + loss_entro_v)  + loss_compensation_v+loss_compensation_v_error_min
...
class bagging_models:
    ...
    def predict(self, method, X, T=[1000.0], lbd=[500.0], sampling=False, n_sample=10):
        ...
        if method in frozenset(('ag', 'myega', 'am', 'cg', 'tvf', 'visoscity_fuse','senond_grade_compensation','visoscity_fix_error_mean','sc_t','self_attention1','self_attention2','self_attention3','self_attention4','self_attention5')):
            for i in range(self.n_models):
...
def att_viscosity_bydomain(bagged_model, ds, method="ag", boundary=7.0):
    """return the error value between predicted and measured viscosities for each point
    ...
    if method == "ag":
        ...
        name_method = "machine composition error"
    elif method == "self_attention1":
        name_method = "attention_1"
    elif method == "self_attention2":
        name_method = "attention_2"
    elif method == "self_attention3":
        name_method = "attention_3"
    elif method == "self_attention4":
        name_method = "attention_4"
    elif method == "self_attention5":
        name_method = "attention_5"
...
np.savetxt("./each_point/Deepmelt-new/Deepmelt_attentional_train_with_fragility_"+str(seed-162)+".csv",train_result_data,delimiter=",")
np.savetxt("./each_point/Deepmelt-new/Deepmelt_attentional_valid_with_fragility_"+str(seed-162)+".csv",valid_result_data,delimiter=",")
np.savetxt("./each_point/Deepmelt-new/Deepmelt_attentional_test_with_fragility_"+str(seed-162)+".csv",test_result_data,delimiter=",")
# resultn_visoscity_fuse = error_viscosity_bydomain(neuralmodel, ds, method='visoscity_fuse',boundary=7)
aaa = 1

# np.savetxt("./each_point/Deepmelt-new/Deepmelt_visoscity_attentional_wiec_fuse_point_error_"+str(seed-162)+".csv",resultn_visoscity_fuse.numpy().reshape(-1, 1),delimiter=",")
