...
class model(torch.nn.Module):
    ...
    # def b_cg(self, x):
    #     """B in free volume (CG) equation"""
    #     # return self.b_cg_o(x) + self.parameter_compensation1_output(self.parameter_compensation1(self.b_cg_o(x)))
    #
    #     c11 = self.b_cg_o(x)
    #     for layer in self.parameter_compensation1:  # Feedforward
    #         # c2 = layer(c2)
    #         c11 = self.dropout(self.activation_function(layer(c11)))
    #     c11 = self.parameter_compensation1_output(c11)
    #
    #     # c12 = self.b_cg_o(x)
    #     # for layer in self.scale_compensation1:  # Feedforward
    #     #     # c2 = layer(c2)
    #     #     c12 = self.dropout(self.activation_function(layer(c12)))
    #     # c12 = self.scale_compensation1_output(c12)
    #
    #     # return self.b_cg_o(x) + torch.log(torch.pow(c11,2)), torch.log(torch.pow(c11,2))#,c11
    #
    #     # return self.b_cg_o(x) + torch.abs(self.lambda1)* torch.tanh(c11), torch.exp(torch.abs(self.lambda1* torch.tanh(c11)))  # ,c11
    #
    #     return self.b_cg_o(x) + torch.abs(self.lambda1) * torch.tanh(c11), torch.abs(self.lambda1 * torch.tanh(c11))
    # def b_cg(self, x):
    #     """B in free volume (CG) equation"""
    #     # return self.b_cg_o(x) + self.parameter_compensation1_output(self.parameter_compensation1(self.b_cg_o(x)))
    #
    ...
    #     # c12 = self.scale_compensation1_output(c12)
    #
    #     return self.b_cg_fix(x)[0]# * c12

    # def b_cg_fuse(self,x,T):
    #
    #     # b_cg = torch.add(self.b_cg(x),(self.first_grade_compensation(x,T)[:,0]).reshape(-1,1))/2
    #     # b_cg = torch.add(self.b_cg(x),(self.first_grade_compensation(x,T)[:,0]).reshape(-1,1))
    #     # b_cg = torch.add(self.b_cg(x), torch.exp((self.first_grade_compensation(x, T)[:, 0]).reshape(-1, 1)))
    #     # b_cg = torch.add(self.b_cg(x), torch.exp((-self.first_grade_compensation(x, T)[:, 0]).reshape(-1, 1)))
    #     b_cg = torch.add(self.b_cg(x), torch.tanh((self.first_grade_compensation(x, T)[:, 0]).reshape(-1, 1)))
    #     # b_cg = torch.add(self.b_cg(x), torch.mul(self.first_grade_compensation(x, T)[:, 0],self.first_grade_compensation_level(x, T)[:, 0]).reshape(-1, 1))
    #     # b_cg = torch.add(self.b_cg(x), torch.mul(self.first_grade_compensation(x, T)[:, 0],
    #     #                                          torch.tanh(self.first_grade_compensation_level(x, T)[:, 0]).reshape(-1, 1)))
    #     return b_cg
    ...
    # def b_tvf(self, x):
    #     # return self.b_tvf_o(x)+self.parameter_compensation2_output(self.parameter_compensation2(self.b_tvf_o(x)))
    #     c11 = self.b_tvf_o(x)
    #     for layer in self.parameter_compensation2:  # Feedforward
    #         # c2 = layer(c2)
    #         c11 = self.dropout(self.activation_function(layer(c11)))
    #     c11 = self.parameter_compensation2_output(c11)
    #
    #     # c12 = self.b_tvf_o(x)
    #     # for layer in self.scale_compensation2:  # Feedforward
    #     #     # c2 = layer(c2)
    #     #     c12 = self.dropout(self.activation_function(layer(c12)))
    #     # c12 = self.scale_compensation2_output(c12)
    #
    #     # return self.b_tvf_o(x) + torch.abs(self.lambda2)* torch.tanh(c11), torch.exp(torch.abs(self.lambda2* torch.tanh(c11)))
    #     return self.b_tvf_o(x) + torch.abs(self.lambda2) * torch.tanh(c11), torch.abs(self.lambda2 * torch.tanh(c11))
    # def b_tvf(self, x):
    #
    #     return self.b_tvf_fix(x)[0] #* c12

    # def b_tvf_fuse(self,x,T):
    #
    #
    #     # b_tvf = torch.add(self.b_tvf(x), (self.first_grade_compensation(x, T)[:, 1]).reshape(-1, 1)) / 2
    #     # b_tvf = torch.add(self.b_tvf(x), (self.first_grade_compensation(x, T)[:, 1]).reshape(-1, 1))
    #     b_tvf = torch.add(self.b_tvf(x), torch.tanh((self.first_grade_compensation(x, T)[:, 1]).reshape(-1, 1)))
    #     # b_tvf = torch.add(self.b_tvf(x), torch.exp((-self.first_grade_compensation(x, T)[:, 1]).reshape(-1, 1)))
    #     # b_tvf = torch.add(self.b_tvf(x), torch.mul(self.first_grade_compensation(x, T)[:, 1],self.first_grade_compensation_level(x, T)[:, 1]).reshape(-1, 1))
    #     # b_tvf = torch.add(self.b_tvf(x), torch.mul(self.first_grade_compensation(x, T)[:, 1],
    #     #                                            torch.tanh(self.first_grade_compensation_level(x, T)[:, 1]).reshape(-1, 1)))
    #     return b_tvf
    ...
    #     """Be term in Adam-Gibbs eq given Ae, Tg and Scong(Tg)"""
    #     # a = self.be_o(x)
    #     # b = self.parameter_compensation3(self.be_o(x))
    #     # c = self.parameter_compensation3_output(self.parameter_compensation3(self.be_o(x)))
    #     # return torch.add(self.be_o(x),self.parameter_compensation3_output(self.parameter_compensation3(self.be_o(x))))
    #     c11 = self.be_o(x)
    #     for layer in self.parameter_compensation3:  # Feedforward
    #         # c2 = layer(c2)
    #         c11 = self.dropout(self.activation_function(layer(c11)))
    #     c11 = self.parameter_compensation3_output(c11)
    #
    #     # c12 = self.be_o(x)
    #     # for layer in self.scale_compensation3:  # Feedforward
    #     #     # c2 = layer(c2)
    #     #     c12 = self.dropout(self.activation_function(layer(c12)))
    #     # c12 = self.scale_compensation2_output(c12)
    #
    #     # return self.be_o(x) + torch.abs(self.lambda3)* torch.tanh(c11), torch.exp(torch.abs(self.lambda3* torch.tanh(c11)))
    #     return self.be_o(x) + torch.abs(self.lambda3) * torch.tanh(c11), torch.abs(self.lambda3 * torch.tanh(c11))

    # def be(self, x):
    #     """Be term in Adam-Gibbs eq given Ae, Tg and Scong(Tg)"""
    #
    #     return self.be_fix(x)[0] #* c12

    # def be_fuse(self,x,T):
    #     # be = torch.add(self.be(x), (self.first_grade_compensation(x, T)[:, 2]).reshape(-1, 1)) / 2
    #     # be = torch.add(self.be(x), (self.first_grade_compensation(x, T)[:, 2]).reshape(-1, 1))
    #     be = torch.add(self.be(x), torch.tanh((self.first_grade_compensation(x, T)[:, 2]).reshape(-1, 1)))
    #     # be = torch.add(self.be(x), torch.exp((-self.first_grade_compensation(x, T)[:, 2]).reshape(-1, 1)))
    #     # be = torch.add(self.be(x), torch.mul(self.first_grade_compensation(x, T)[:, 2],self.first_grade_compensation_level(x, T)[:, 2]).reshape(-1, 1))
    #     # be = torch.add(self.be(x), torch.mul(self.first_grade_compensation(x, T)[:, 2],
    #     #                                      torch.tanh(self.first_grade_compensation_level(x, T)[:, 2]).reshape(-1, 1)))
    #     return be
    ...
    #     """Melt configurational entropy at temperature T
    #     """
    #     # return self.sc_t_o(x,T) + self.parameter_compensation4_output(self.parameter_compensation4(self.sc_t_o(x,T)))
    #
    #     c11 = self.sc_t_o(x, T)
    #     for layer in self.parameter_compensation4:  # Feedforward
    #         # c2 = layer(c2)
    #         c11 = self.dropout(self.activation_function(layer(c11)))
    #     c11 = self.parameter_compensation4_output(c11)
    #
    #     # return self.sc_t_o(x, T) + torch.abs(self.lambda4)* torch.tanh(c11), torch.exp(torch.abs(self.lambda4* torch.tanh(c11)))
    #
    #     return self.sc_t_o(x, T) + torch.abs(self.lambda4) * torch.tanh(c11), torch.abs(self.lambda4 * torch.tanh(c11))
    ...
    #
    #
    #     return self.sc_t_fix(x, T)[0] #* c12
    ...
    # def sc_t_fuse(self, x, T):
    #     """
    #     """
    #     # sc_t = torch.add(self.sc_t(x,T), (self.first_grade_compensation(x, T)[:, 3]).reshape(-1, 1)) / 2
    #     # sc_t = torch.add(self.sc_t(x, T), (self.first_grade_compensation(x, T)[:, 3]).reshape(-1, 1))
    #     # sc_t = torch.add(self.sc_t(x, T), torch.exp((-self.first_grade_compensation(x, T)[:, 3])).reshape(-1, 1))
    #     sc_t = torch.add(self.sc_t(x, T), torch.tanh((self.first_grade_compensation(x, T)[:, 3])).reshape(-1, 1)) /2
    #     # sc_t = torch.add(self.sc_t(x, T), torch.mul(self.first_grade_compensation(x, T)[:, 3],self.first_grade_compensation_level(x, T)[:, 3]).reshape(-1, 1))
    #     # sc_t = torch.add(self.sc_t(x, T), torch.mul(self.first_grade_compensation(x, T)[:, 3],torch.tanh(self.first_grade_compensation_level(x, T)[:, 3]).reshape(-1, 1)))
    #     return sc_t
    ...
    # def first_grade_compensation(self,x,T):
    #     """first comensation network for mid-variance"""
    #     ae = self.ae(x)
    #     tg = self.tg(x)
    #     a_am = self.a_am(x)
    #     a_cg = self.a_cg(x)
    ...
    #     c_cg = self.c_cg(x)
    #     c_tvf = self.c_tvf(x)
    #     fragility = self.fragility(x)
    #     sctg = self.sctg(x)
    #     dCp = self.dCp(x,T)
    #     c1 = torch.cat([ae,tg,a_am,a_cg,a_tvf,to_cg,c_cg,c_tvf,fragility,sctg,dCp],1)
    #     for layer in self.machine_compensation1: # Feedforward
    #         # c1 = layer(c1)
    #         c1 = self.dropout(self.activation_function(layer(c1)))
    #     c1 = self.machine_compensation1_output(c1)
    #     return c1
    ...
    def senond_grade_compensation(self, x, T):
        ...
        # c22 =torch.mul(c21, c22)
        #
        #
        # for layer in self.machine_compensation2:  # Feedforward
        #     # c2 = layer(c2)
        #     c22 = self.dropout(self.activation_function(layer(c22)))
        # c22 = self.machine_compensation2_output(c22)

        # for layer in self.scale_compensation22:  # Feedforward
        #     # c2 = layer(c2)
        #     c22 = self.dropout(self.activation_function(layer(c22)))
        # c22 = self.scale_compensation22_output(c22)
    ...
    #     sctg = self.sctg(x)
    #     dCp = self.dCp(x, T)
    #     b_cg = self.b_cg_fuse(x,T)
    #     b_tvf = self.b_tvf_fuse(x,T)
    #     be = self.be_fuse(x,T)
    #     sc_t = self.sc_t_fuse(x,T)
    #     c2 = torch.cat([ae, tg, a_am, a_cg, a_tvf, to_cg, c_cg, c_tvf, fragility, sctg, dCp,b_cg,b_tvf,be,sc_t], 1)
    #     """first comensation network for visosity"""
    #     for layer in self.machine_compensation2: # Feedforward
    #         # c2 = layer(c2)
    #         c2 = self.dropout(self.activation_function(layer(c2)))
    #     c2 = self.machine_compensation2_output(c2)
    #     return c2

    # def first_grade_compensation_level(self,x,T):
    #     """first comensation network for mid-variance"""
    #     ae = self.ae(x)
    #     tg = self.tg(x)
    #     a_am = self.a_am(x)
    ...
    #     dCp = self.dCp(x,T)
    #     c1 = torch.cat([ae,tg,a_am,a_cg,a_tvf,to_cg,c_cg,c_tvf,fragility,sctg,dCp],1)
    #     for layer in self.machine_compensation1_level: # Feedforward
    #         # c1 = layer(c1)
    #         c1 = self.dropout(self.activation_function(layer(c1)))
    #     c1 = self.machine_compensation1_level_output(c1)
    #     return c1

    # def senond_grade_compensation_level(self, x, T):
    #     ae = self.ae(x)
    #     tg = self.tg(x)
    #     a_am = self.a_am(x)
    #     a_cg = self.a_cg(x)
    ...
    #     c2 = torch.cat([ae, tg, a_am, a_cg, a_tvf, to_cg, c_cg, c_tvf, fragility, sctg, dCp,b_cg,b_tvf,be,sc_t], 1)
    #     """first comensation network for visosity"""
    #     for layer in self.machine_compensation2_level: # Feedforward
    #         # c2 = layer(c2)
    #         c2 = self.dropout(self.activation_function(layer(c2)))
    #     c2 = self.machine_compensation2_level_output(c2)
    #     return c2
    ...
    def fuse(self, x, T):
        ...
        # return torch.add(v, fix_v), self.activation_function(fix_v)
        # return torch.add(v, torch.abs(self.lambda5) * torch.tanh(fix_v)), torch.exp(torch.abs(self.lambda5 * torch.tanh(fix_v)))
        return torch.add(v, torch.abs(self.lambda5) * torch.tanh(fix_v)), torch.abs(self.lambda5 * torch.tanh(fix_v))
        # return torch.add(v,fix_v),torch.pow(self.activation_function(fix_v),0.1)
        # return torch.add(v, fix_v), torch.tanh(self.activation_function(fix_v))
    ...
    # def visoscity_fuse(self, x, T):
    #     # return torch.add(
    #     #     torch.add(torch.add(torch.add(torch.add(self.ag(x, T), self.myega(x, T)), self.am(x, T)), self.cg(x, T)),
    #     #               self.tvf(x, T)) , torch.tanh(self.senond_grade_compensation(x, T)))/ 5
    #     return torch.add(
    #         torch.add(torch.add(torch.add(torch.add(self.ag(x, T), self.myega(x, T)), self.am(x, T)), self.cg(x, T)),
    #                   self.tvf(x, T)) / 5, self.senond_grade_compensation(x, T))
    #     # return torch.add(
    #     #     torch.add(torch.add(torch.add(torch.add(self.ag(x, T), self.myega(x, T)), self.am(x, T)), self.cg(x, T)),
    #     #               self.tvf(x, T)) / 5, torch.mul(self.senond_grade_compensation(x, T),self.senond_grade_compensation_level(x, T)))
...
class bagging_models:
    ...
    def predict(self, method, X, T=[1000.0], lbd=[500.0], sampling=False, n_sample=10):
        ...
        if method in frozenset(('ag', 'myega', 'am', 'cg', 'tvf', 'visoscity_fuse')):
            for i in range(self.n_models):
...
RMSE_ag_tot, RMSE_ag_low, RMSE_ag_high = RMSE_viscosity_bydomain(neuralmodel, ds, method='ag',
                                                                                  boundary=7)
RMSE_cg_tot, RMSE_cg_low, RMSE_cg_high = RMSE_viscosity_bydomain(neuralmodel, ds, method='cg',
                                                                                  boundary=7)
RMSE_tvf_tot, RMSE_tvf_low, RMSE_tvf_high = RMSE_viscosity_bydomain(neuralmodel, ds, method='tvf',
                                                                                     boundary=7)
RMSE_am_tot, RMSE_am_low, RMSE_am_high = RMSE_viscosity_bydomain(neuralmodel, ds, method='am',
                                                                                  boundary=7)
RMSE_myega_tot, RMSE_myega_low, RMSE_myega_high = RMSE_viscosity_bydomain(neuralmodel, ds,
                                                                                           method='myega', boundary=7)
RMSE_compensation_tot, RMSE_compensation_low, RMSE_compensation_high = RMSE_viscosity_bydomain(
...
# resultn_ag = error_viscosity_bydomain(neuralmodel, ds, method='ag',boundary=7)
# resultn_cg = error_viscosity_bydomain(neuralmodel, ds, method='cg',boundary=7)
# resultn_tvf = error_viscosity_bydomain(neuralmodel, ds, method='tvf',boundary=7)
# resultn_am = error_viscosity_bydomain(neuralmodel, ds, method='am',boundary=7)
# resultn_myega = error_viscosity_bydomain(neuralmodel, ds, method='myega',boundary=7)
resultn_visoscity_fuse = error_viscosity_bydomain(neuralmodel, ds, method='visoscity_fuse',boundary=7)
aaa = 1
# np.savetxt("./each_point/Deepmelt/Deepmelt_ag_avg_wiec_point_error_"+str(seed-162)+".csv",resultn_ag.numpy().reshape(-1, 1),delimiter=",")
# np.savetxt("./each_point/Deepmelt/Deepmelt_cg_avg_wiec_point_error_"+str(seed-162)+".csv",resultn_cg.numpy().reshape(-1, 1),delimiter=",")
# np.savetxt("./each_point/Deepmelt/Deepmelt_tvf_avg_wiec_point_error_"+str(seed-162)+".csv",resultn_tvf.numpy().reshape(-1, 1),delimiter=",")
# np.savetxt("./each_point/Deepmelt/Deepmelt_am_avg_wiec_point_error_"+str(seed-162)+".csv",resultn_am.numpy().reshape(-1, 1),delimiter=",")
# np.savetxt("./each_point/Deepmelt/Deepmelt_myega_avg_wiec_point_error_"+str(seed-162)+".csv",resultn_myega.numpy().reshape(-1, 1),delimiter=",")
np.savetxt("./each_point/Deepmelt/Deepmelt_visoscity_avg_wiec_fuse_point_error_DR=0.05_NR_"+str(seed-162)+".csv",resultn_visoscity_fuse.numpy().reshape(-1, 1),delimiter=",")
...
#                                           neuralmodel.predict("sellmeier", ds.x_ri_train, lbd=ds.lbd_ri_train).mean(
#                                               axis=1)))
# ese_ri_valid = np.sqrt(mean_squared_error(ds.y_ri_valid.cpu().detach().numpy(),
#                                           neuralmodel.predict("sellmeier", ds.x_ri_valid, lbd=ds.lbd_ri_valid).mean(
#                                               axis=1)))
# ese_ri_test = np.sqrt(mean_squared_error(ds.y_ri_test.cpu().detach().numpy(),
#                                          neuralmodel.predict("sellmeier", ds.x_ri_test, lbd=ds.lbd_ri_test).mean(
#                                              axis=1)))

# print('RMSE density train after training {0:.4f}'.format(ese_density_train))
# print('RMSE density valid after training {0:.4f}'.format(ese_density_valid))
# print('RMSE density test after training {0:.4f}'.format(ese_density_test))
# print('')
#
# print('RMSE refractive train loss after training {0:.4f}'.format(ese_ri_train))
# print('RMSE refractive valid loss after training {0:.4f}'.format(ese_ri_valid))
# print('RMSE refractive test loss after training {0:.4f}'.format(ese_ri_test))
# print('')
#
# Raman_error_train = [0] * ds.x_raman_train.shape[0]
# Raman_error_valid = [0] * ds.x_raman_valid.shape[0]
# for i in range(ds.x_raman_train.shape[0]):
#     Raman_error_train[i] = np.mean(np.sqrt(
#         (raman_train_predictions[i, :] - ds.y_raman_train[i, :].detach().numpy()) ** 2) / raman_train_predictions[i, :])
#
# for i in range(ds.x_raman_valid.shape[0]):
#     Raman_error_valid[i] = np.mean(np.sqrt(
#         (raman_valid_predictions[i, :] - ds.y_raman_valid[i, :].detach().numpy()) ** 2) / raman_valid_predictions[i, :])
#
# print('RMSE raman train loss after training {0:.4f}'.format(np.mean(Raman_error_train[0])))
# print('RMSE raman valid loss after training {0:.4f}'.format(np.mean(Raman_error_valid[0])))
