...
#The seed range is from 162-171
seed = 171
# random.seed(seed)
np.random.seed(seed)
...
class model(torch.nn.Module):
    ...
    def __init__(self, input_size, hidden_size, num_layers, nb_channels_raman, p_drop=0.5,
        ...
        self.input_size_c1 = 1
        self.input_size_c2 = 5
        ...
        self.out_thermo = torch.nn.Linear(self.hidden_size, 11)  # Linear output
        self.out_raman = torch.nn.Linear(self.hidden_size, self.nb_channels_raman)  # Linear output

        self.lambda1 = torch.nn.Parameter(torch.ones(1, 1))
        self.lambda2 = torch.nn.Parameter(torch.ones(1, 1))
        self.lambda3 = torch.nn.Parameter(torch.ones(1, 1))
        self.lambda4 = torch.nn.Parameter(torch.ones(1, 1))
        self.lambda5 = torch.nn.Parameter(torch.ones(1, 1))

        # self.machine_compensation1 = torch.nn.ModuleList([torch.nn.Linear(self.input_size_c1, self.hidden_size)])
        # self.machine_compensation1.extend([torch.nn.Linear(self.hidden_size, self.hidden_size) for i in range(1, self.num_layers)])
        # self.machine_compensation1_output = torch.nn.Linear(self.hidden_size, 4)

        # self.machine_compensation1_level = torch.nn.ModuleList([torch.nn.Linear(self.input_size_c1, self.hidden_size)])
        # self.machine_compensation1_level.extend(
        #     [torch.nn.Linear(self.hidden_size, self.hidden_size) for i in range(1, self.num_layers)])
        # self.machine_compensation1_level_output = torch.nn.Linear(self.hidden_size, 4)

        self.parameter_compensation1 = torch.nn.ModuleList([torch.nn.Linear(self.input_size_c1, self.hidden_size)])
        self.parameter_compensation2 = torch.nn.ModuleList([torch.nn.Linear(self.input_size_c1, self.hidden_size)])
        self.parameter_compensation3 = torch.nn.ModuleList([torch.nn.Linear(self.input_size_c1, self.hidden_size)])
        self.parameter_compensation4 = torch.nn.ModuleList([torch.nn.Linear(self.input_size_c1, self.hidden_size)])

        self.parameter_compensation1.extend(
            [torch.nn.Linear(self.hidden_size, self.hidden_size) for i in range(1, self.num_layers)])

        self.parameter_compensation1_output = torch.nn.Linear(self.hidden_size, 1)

        self.parameter_compensation2.extend(
            [torch.nn.Linear(self.hidden_size, self.hidden_size) for i in range(1, self.num_layers)])

        self.parameter_compensation2_output = torch.nn.Linear(self.hidden_size, 1)

        self.parameter_compensation3.extend(
            [torch.nn.Linear(self.hidden_size, self.hidden_size) for i in range(1, self.num_layers)])

        self.parameter_compensation3_output = torch.nn.Linear(self.hidden_size, 1)

        self.parameter_compensation4.extend(
            [torch.nn.Linear(self.hidden_size, self.hidden_size) for i in range(1, self.num_layers)])

        self.parameter_compensation4_output = torch.nn.Linear(self.hidden_size, 1)

        # self.scale_compensation1 = torch.nn.ModuleList([torch.nn.Linear(self.input_size_c1, self.hidden_size)])
        # self.scale_compensation2 = torch.nn.ModuleList([torch.nn.Linear(self.input_size_c1, self.hidden_size)])
        # self.scale_compensation3 = torch.nn.ModuleList([torch.nn.Linear(self.input_size_c1, self.hidden_size)])
        # self.scale_compensation4 = torch.nn.ModuleList([torch.nn.Linear(self.input_size_c1, self.hidden_size)])
        #
        # self.scale_compensation1.extend(
        #     [torch.nn.Linear(self.hidden_size, self.hidden_size) for i in range(1, self.num_layers)])
        #
        # self.scale_compensation1_output = torch.nn.Linear(self.hidden_size, 1)
        #
        # self.scale_compensation2.extend(
        #     [torch.nn.Linear(self.hidden_size, self.hidden_size) for i in range(1, self.num_layers)])
        #
        # self.scale_compensation2_output = torch.nn.Linear(self.hidden_size, 1)
        #
        # self.scale_compensation3.extend(
        #     [torch.nn.Linear(self.hidden_size, self.hidden_size) for i in range(1, self.num_layers)])
        #
        # self.scale_compensation3_output = torch.nn.Linear(self.hidden_size, 1)
        #
        # self.scale_compensation4.extend(
        #     [torch.nn.Linear(self.hidden_size, self.hidden_size) for i in range(1, self.num_layers)])
        #
        # self.scale_compensation4_output = torch.nn.Linear(self.hidden_size, 1)

        self.machine_compensation2 = torch.nn.ModuleList([torch.nn.Linear(self.input_size_c2, self.hidden_size)])

        self.machine_compensation2.extend(
            [torch.nn.Linear(self.hidden_size, self.hidden_size) for i in range(1, self.num_layers)])
        # self.machine_compensation1.extend([torch.nn.Linear(self.hidden_size, 1)])
        self.machine_compensation2_output = torch.nn.Linear(self.hidden_size, 1)

        # self.scale_compensation22 = torch.nn.ModuleList([torch.nn.Linear(self.input_size_c2, self.hidden_size)])
        #
        # self.scale_compensation22.extend(
        #     [torch.nn.Linear(self.hidden_size, self.hidden_size) for i in range(1, self.num_layers)])
        # # self.machine_compensation1.extend([torch.nn.Linear(self.hidden_size, 1)])
        # self.scale_compensation22_output = torch.nn.Linear(self.hidden_size, 1)

        self.machine_compensation_self_attention = torch.nn.ModuleList([torch.nn.Linear(5, self.hidden_size)])

        self.machine_compensation_self_attention.extend(
            [torch.nn.Linear(self.hidden_size, self.hidden_size) for i in range(1, self.num_layers)])
        # self.machine_compensation1.extend([torch.nn.Linear(self.hidden_size, 1)])
        self.machine_compensation_self_attention_output = torch.nn.Linear(self.hidden_size, 5)

        # self.machine_compensation2_level = torch.nn.ModuleList([torch.nn.Linear(self.input_size_c2, self.hidden_size)])
        #
        # self.machine_compensation2_level.extend(
        #     [torch.nn.Linear(self.hidden_size, self.hidden_size) for i in range(1, self.num_layers)])
        # # self.machine_compensation1.extend([torch.nn.Linear(self.hidden_size, 1)])
        # self.machine_compensation2_level_output = torch.nn.Linear(self.hidden_size, 1)

        self.machine_compensation_fix_v = torch.nn.ModuleList([torch.nn.Linear(1, self.hidden_size)])

        self.machine_compensation_fix_v.extend(
            [torch.nn.Linear(self.hidden_size, self.hidden_size) for i in range(1, self.num_layers)])
        # self.machine_compensation1.extend([torch.nn.Linear(self.hidden_size, 1)])
        self.machine_compensation_fix_v_output = torch.nn.Linear(self.hidden_size, 1)
    ...
    def output_bias_init(self):
        ...
        self.out_thermo.bias = torch.nn.Parameter(data=torch.tensor([np.log(1000.), np.log(10.),  # Tg, ScTg
                                                                     ...
                                                                     np.log(500.), np.log(100.), np.log(400.),
                                                                     # To_CG, C_CG, C_TVF
    ...
    def aCpl(self, x):
        ...
        """
        out = 81.37 * x[:, 0] + 27.21 * x[:, 1] + 100.6 * x[:, 2] + 50.13 * x[:, 3] + x[:, 0] * (
                    x[:, 3] * x[:, 3]) * 151.7
        return torch.reshape(out, (out.shape[0], 1))
    ...
    def b_cg(self, x):
        ...
        return 0.5 * (12.0 - self.a_cg(x)) * (self.tg(x) - self.to_cg(x) + torch.sqrt(
    ...
    def myega(self, x, T):
        ...
        """
        return self.ae(x) + (12.0 - self.ae(x)) * (self.tg(x) / T) * torch.exp(
            (self.fragility(x) / (12.0 - self.ae(x)) - 1.0) * (self.tg(x) / T - 1.0))
    ...
    def cg(self, x, T):
        ...
        """
        return self.a_cg(x) + 2.0 * self.b_cg(x) / (
                    T - self.to_cg(x) + torch.sqrt((T - self.to_cg(x)) ** 2 + self.c_cg(x) * T))
    ...
    def senond_grade_compensation(self, x, T):
        c22 = torch.add(torch.add(torch.add(torch.add(self.ag(x, T), self.myega(x, T)), self.am(x, T)), self.cg(x, T)),
                        self.tvf(x, T)) / 5
    ...
    def fuse(self, x, T):
        v = self.senond_grade_compensation(x, T)
        fix_v = self.senond_grade_compensation(x, T)
        for layer in self.machine_compensation_fix_v:  # Feedforward
            # c2 = layer(c2)
            fix_v = self.dropout(self.activation_function(layer(fix_v)))
        fix_v = self.machine_compensation_fix_v_output(fix_v)


        return torch.add(v, torch.abs(self.lambda5) * torch.tanh(fix_v)), torch.abs(self.lambda5 * torch.tanh(fix_v))


    def visoscity_fuse(self, x, T):
        a = self.fuse(x, T)[0]
        return a

    def visoscity_fix_error_mean(self, x, T):
        a = self.fuse(x,T)[1]
        return a.mean()

    def visoscity_fix_error(self, x, T):
        a = self.fuse(x,T)[1]
        return a

    def m1_error(self, x, T):
        e1 = self.b_cg(x)
        e2 = self.be(x)
        e3 = self.b_tvf(x)
        e4 = self.sc_t(x,T)
        return e1.mean()+e2.mean()+e3.mean()+e4.mean()
...
def training(neuralmodel, ds, criterion, optimizer, save_switch=True, save_name="./temp", train_patience=50,
             min_delta=0.1, verbose=True, mode="main", max_epochs=5000):
    ...
    while val_ex <= train_patience:
        ...
        y_compensation_pre_train = neuralmodel.visoscity_fuse(ds.x_visco_train, ds.T_visco_train)
        y_compensation_train_error =neuralmodel.visoscity_fix_error(ds.x_visco_train, ds.T_visco_train)

        y_compensation_train_m1_error = neuralmodel.m1_error(ds.x_visco_train, ds.T_visco_train)
        ...
        y_compensation_pre_valid = neuralmodel.visoscity_fuse(ds.x_visco_valid, ds.T_visco_valid)
        y_compensation_valid_error =neuralmodel.visoscity_fix_error(ds.x_visco_valid, ds.T_visco_valid)

        y_compensation_valid_m1_error = neuralmodel.m1_error(ds.x_visco_valid, ds.T_visco_valid)
        ...
        loss_compensation = criterion(y_compensation_pre_train, ds.y_visco_train)

        loss_compensation_error_min  = y_compensation_train_error
        loss_compensation_m1_error_min  = y_compensation_train_m1_error

        if mode == "pretrain":
            # loss = loss_tg + loss_raman + loss_density + loss_entro + loss_ri
            loss = loss_tg +  loss_entro
        else:
            # loss = loss_ag + loss_myega + loss_am + loss_cg + loss_tvf + loss_raman + loss_density + loss_entro + loss_ri + loss_compensation
            loss = loss_ag + loss_myega + loss_am + loss_cg + loss_tvf  + loss_entro +  loss_compensation
        ...
        with torch.set_grad_enabled(False):
            ...
            loss_compensation_v = criterion(y_compensation_pre_valid, ds.y_visco_valid)

            loss_compensation_v_error_min = y_compensation_valid_error

            loss_m1_error = y_compensation_valid_m1_error

            if mode == "pretrain":
                # loss_v = loss_tg_v + loss_raman_v + loss_density_v + loss_entro_v + loss_ri_v
                loss_v = loss_tg_v  + loss_entro_v
            else:
                loss_v = loss_ag_v + loss_myega_v + loss_am_v + loss_cg_v + loss_tvf_v + loss_entro_v  + loss_compensation_v
    ...
    if verbose == True:
        ...
        print(
            'Scaled valid loss values are {:.2f}, {:.2f}, {:.2f}, {:.2f} for Tg, entropy, viscosity (AG)，viscosity'.format(
                loss_tg_v, loss_entro_v, loss_ag_v, loss_compensation_v
            ))

    return neuralmodel, record_train_loss, record_valid_loss


def training2(neuralmodel, ds, criterion, optimizer, save_switch=True, save_name="./temp", nb_folds=10,
              train_patience=50, min_delta=0.1, verbose=True, mode="main", device='cuda'):
    """train neuralmodel given a dataset, criterion and optimizer
    ...
    while val_ex <= train_patience:
        ...
        for i in range(nb_folds):  # loop for K-Fold training to reduce memory footprint
            ...
            y_conpensation_pred_train = neuralmodel.visoscity_fuse(x_visco_train, T_visco_train)
            ...
            loss_density = precision_density * criterion(y_density_pred_train,
                                                         y_density_train)  # + neuralmodel.log_vars[2]
            ...
            loss_compensation = precision_visco * criterion(y_conpensation_pred_train, y_visco_train)

            if mode == "pretrain":
                loss_fold = loss_tg + loss_raman + loss_density + loss_entro + loss_ri
            else:
                loss_fold = (loss_ag + loss_myega + loss_am + loss_cg +
                             loss_tvf + loss_raman + loss_density + loss_entro + loss_ri + loss_compensation)
        ...
        with torch.set_grad_enabled(False):
            ...
            y_compensation_pred_valid = neuralmodel.visoscity_fuse(ds.x_visco_valid.to(device),
                                                                   ds.T_visco_valid.to(device))

            # validation loss
            loss_ag_v = precision_visco * criterion(y_ag_pred_valid,
                                                    ds.y_visco_valid.to(device))  # + neuralmodel.log_vars[0]
            loss_myega_v = precision_visco * criterion(y_myega_pred_valid,
                                                       ds.y_visco_valid.to(device))  # + neuralmodel.log_vars[0]
            loss_am_v = precision_visco * criterion(y_am_pred_valid,
                                                    ds.y_visco_valid.to(device))  # + neuralmodel.log_vars[0]
            loss_cg_v = precision_visco * criterion(y_cg_pred_valid,
                                                    ds.y_visco_valid.to(device))  # + neuralmodel.log_vars[0]
            loss_tvf_v = precision_visco * criterion(y_tvf_pred_valid,
                                                     ds.y_visco_valid.to(device))  # + neuralmodel.log_vars[0]
            loss_raman_v = precision_raman * criterion(y_raman_pred_valid,
                                                       ds.y_raman_valid.to(device))  # + neuralmodel.log_vars[1]
            loss_density_v = precision_density * criterion(y_density_pred_valid,
                                                           ds.y_density_valid.to(device))  # + neuralmodel.log_vars[2]
            loss_entro_v = precision_entro * criterion(y_entro_pred_valid,
                                                       ds.y_entro_valid.to(device))  # + neuralmodel.log_vars[3]
            ...
            loss_compensation_v = precision_visco * criterion(y_compensation_pred_valid, ds.y_visco_valid.to(device))

            if mode == "pretrain":
                loss_v = loss_tg_v + loss_raman_v + loss_density_v + loss_entro_v + loss_ri_v
            else:
                loss_v = loss_ag_v + loss_myega_v + loss_am_v + loss_cg_v + loss_tvf_v + loss_raman_v + loss_density_v + loss_entro_v + loss_ri_v + loss_compensation_v
    ...
    if verbose == True:
        ...
        print("Running time in seconds:", time2 - time1)
        print(
            'Scaled valid loss values are {:.2f}, {:.2f}, {:.2f}, {:.2f}, {:.2f}, {:.2f} , {:.2f} for Tg, Raman, density, entropy, ri, viscosity (AG), viscpsity'.format(
                loss_tg_v, loss_raman_v, loss_density_v, loss_entro_v, loss_ri_v, loss_ag_v, loss_compensation_v
            ))
...
class bagging_models:
    ...
    def predict(self, method, X, T=[1000.0], lbd=[500.0], sampling=False, n_sample=10):
        ...
        if method in frozenset(('ag', 'myega', 'am', 'cg', 'tvf', 'visoscity_fuse', 'senond_grade_compensation',
                                'visoscity_fix_error_mean', 'sc_t', 'self_attention1', 'self_attention2',
                                'self_attention3', 'self_attention4', 'self_attention5')):
            for i in range(self.n_models):
...
def load_pretrained_bagged(path_viscosity="./data/NKAS_viscosity_reference.hdf5", path_raman="./data/NKAS_Raman.hdf5",
                           path_density="./data/NKAS_density.hdf5", path_optical="./data/NKAS_optical.hdf5",
                           path_models="./model/best/", device=torch.device('cpu')):
...
def RMSE_viscosity_bydomain(bagged_model, ds, method="ag", boundary=7.0):
    ...
    # y_pred_train_all = bagged_model.predict(method, ds.x_visco_train, ds.T_visco_train)
    # y_pred_valid_all = bagged_model.predict(method, ds.x_visco_valid, ds.T_visco_valid)
    # y_pred_test_all = bagged_model.predict(method, ds.x_visco_test, ds.T_visco_test)
    ...
    mse_train = np.abs(y_pred_train-y_train.numpy().reshape(-1,1))
    mse_valid = np.abs(y_pred_valid-y_valid.numpy().reshape(-1,1))
    mse_test = np.abs(y_pred_test-y_test.numpy().reshape(-1,1))
    ...
    elif method == "visoscity_fuse":
        name_method = "machine composition"
    elif method == "senond_grade_compensation":
        name_method = "machine composition without error fix"

    print("Using the equation from {}:".format(name_method))
    print("    RMSE on the full range (0-15 log Pa s): train {0:.4f}, valid {1:.4f}, test {2:.4f}".format(
        total_RMSE_train,
        total_RMSE_valid,
        total_RMSE_test))
    print("    RMSE on the -inf - {:.4f} log Pa s range: train {:.4f}, valid {:.4f}, test {:.4f}".format(boundary,
                                                                                                         low_RMSE_train,
                                                                                                         low_RMSE_valid,
                                                                                                         low_RMSE_test))
    print("    RMSE on the {:.4f} - +inf log Pa s range: train {:.4f}, valid {:.4f}, test {:.4f}".format(boundary,
                                                                                                         high_RMSE_train,
                                                                                                         high_RMSE_valid,
                                                                                                         high_RMSE_test))
    print("")
    return out1, out2, out3,y_pred_train,y_pred_valid,y_pred_test,mse_train, mse_valid, mse_test

def error_viscosity_bydomain(bagged_model, ds, method="ag", boundary=7.0):
    """return the error value between predicted and measured viscosities for each point
    ...
    y_valid = ds.y_visco_valid
    y_test = ds.y_visco_test
    # resultn = abs(y_test.reshape(-1, 1) - y_pred_test.reshape(-1, 1))
    total_RMSE_train = mean_squared_error(y_pred_train, y_train, squared=False)
    total_RMSE_valid = mean_squared_error(y_pred_valid, y_valid, squared=False)
    total_RMSE_test = mean_squared_error(y_pred_test, y_test, squared=False)
    ...
    if method == "ag":
        ...
        name_method = "machine composition"
    elif method == "visoscity_fix_error_mean":
        name_method = "machine composition error"
    elif method == "tg":
        name_method = "parameter tg"
    elif method == "sctg":
        name_method = "parameter sctg"
    elif method == "ae":
        name_method = "parameter ae"
    elif method == "a_am":
        name_method = "parameter a_am"
    elif method == "a_cg":
        name_method = "parameter a_cg"

    elif method == "a_tvf":
        name_method = "parameter a_tvf"
    elif method == "to_cg":
        name_method = "parameter to_cg"
    elif method == "c_cg":
        name_method = "parameter c_cg"
    elif method == "c_tvf":
        name_method = "parameter c_tvf"
    elif method == "fragility":
        name_method = "parameter fragility"
    elif method == "b_cg":
        name_method = "parameter b_cg"
    elif method == "b_tvf":
        name_method = "parameter b_tvf"
    elif method == "be":
        name_method = "parameter be"
    elif method == "sc_t":
        name_method = "parameter sc_t"
...
for i in range(nb_exp):
    ...
    print("...\n")
    name = "./mymodel_model_compensation/candidates/l" + str(nb_layers) + "_n" + str(nb_neurons) + "_p" + str(
        p_drop) + "_m" + str(i) + ".pth"

    # declaring model
    neuralmodel = model(4, nb_neurons, nb_layers, ds.nb_channels_raman, p_drop=p_drop)
    ...
    optimizer = torch.optim.Adam(neuralmodel.parameters(), lr=0.0006, weight_decay=0.01)  # optimizer
    # optimizer = torch.optim.Adam(neuralmodel.parameters(), lr=0.001, weight_decay=0.01)  # optimizer
...
record_loss["loss_compensation_train"] = np.zeros(nb_exp)
record_loss["loss_compensation_valid"] = np.zeros(nb_exp)
...
for i in range(nb_exp):
    name = "./mymodel_model_compensation/candidates/l" + str(nb_layers) + "_n" + str(nb_neurons) + "_p" + str(
        p_drop) + "_m" + str(i) + ".pth"
    record_loss.loc[i, "name"] = "l" + str(nb_layers) + "_n" + str(nb_neurons) + "_p" + str(p_drop) + "_m" + str(
        i) + ".pth"
    ...
    with torch.set_grad_enabled(False):
        ...
        y_compensation_pred_train = neuralmodel.visoscity_fuse(ds.x_visco_train, ds.T_visco_train)
        ...
        y_entro_pred_valid = neuralmodel.sctg(ds.x_entro_valid)
        # y_ri_pred_valid = neuralmodel.sellmeier(ds.x_ri_valid, ds.lbd_ri_valid)
        y_compensation_pred_valid = neuralmodel.visoscity_fuse(ds.x_visco_valid, ds.T_visco_valid)
        ...
        record_loss.loc[i, "loss_compensation_train"] = np.sqrt(
            criterion(y_compensation_pred_train, ds.y_visco_train).item())
        ...
        record_loss.loc[i, "loss_compensation_valid"] = np.sqrt(
            criterion(y_compensation_pred_valid, ds.y_visco_valid).item())
    ...
    record_loss.loc[i, "loss_train"] = (record_loss.loc[i, "loss_ag_train"] +
                                        ...
                                        record_loss.loc[i, "loss_compensation_train"])
    ...
    record_loss.loc[i, "loss_valid"] = (record_loss.loc[i, "loss_ag_valid"] +
                                        ...
                                        record_loss.loc[i, "loss_compensation_valid"]
                                        )
...
for i in best_recorded.loc[:, "name"]:
    shutil.copyfile("./mymodel_model_compensation/candidates/" + i, "./mymodel_model_compensation/best/" + i)

best_recorded.to_csv("./mymodel_model_compensation/best/best_list.csv")
...
ds = data_loader(path_data, path_raman, path_density, path_optical, device)

path = "./mymodel_model_compensation/best/"
name_list = pd.read_csv("./mymodel_model_compensation/best/best_list.csv").loc[:, "name"]

neuralmodel = bagging_models(path, name_list, ds, device)

train_result_data = np.array([0]*1198).reshape(-1,1)
valid_result_data = np.array([0]*261).reshape(-1,1)
test_result_data = np.array([0]*522).reshape(-1,1)


RMSE_ag_tot, RMSE_ag_low, RMSE_ag_high ,ag_train,ag_valid,ag_test,ag_train_diff,ag_valid_diff,ag_test_diff= RMSE_viscosity_bydomain(neuralmodel, ds, method='ag',
                                                                                  boundary=7)
train_result_data = np.concatenate((train_result_data,ag_train),axis=1)
valid_result_data = np.concatenate((valid_result_data,ag_valid),axis=1)
test_result_data = np.concatenate((test_result_data,ag_test),axis=1)

train_result_data = np.concatenate((train_result_data,ag_train_diff),axis=1)
valid_result_data = np.concatenate((valid_result_data,ag_valid_diff),axis=1)
test_result_data = np.concatenate((test_result_data,ag_test_diff),axis=1)

RMSE_cg_tot, RMSE_cg_low, RMSE_cg_high ,cg_train,cg_valid,cg_test,cg_train_diff,cg_valid_diff,cg_test_diff= RMSE_viscosity_bydomain(neuralmodel, ds, method='cg',boundary=7)

train_result_data = np.concatenate((train_result_data,cg_train),axis=1)
valid_result_data = np.concatenate((valid_result_data,cg_valid),axis=1)
test_result_data = np.concatenate((test_result_data,cg_test),axis=1)

train_result_data = np.concatenate((train_result_data,cg_train_diff),axis=1)
valid_result_data = np.concatenate((valid_result_data,cg_valid_diff),axis=1)
test_result_data = np.concatenate((test_result_data,cg_test_diff),axis=1)

RMSE_tvf_tot, RMSE_tvf_low, RMSE_tvf_high,tvf_train,tvf_valid,tvf_test,tvf_train_diff,tvf_valid_diff,tvf_test_diff = RMSE_viscosity_bydomain(neuralmodel, ds, method='tvf',
                                                                                     boundary=7)
train_result_data = np.concatenate((train_result_data,tvf_train),axis=1)
valid_result_data = np.concatenate((valid_result_data,tvf_valid),axis=1)
test_result_data = np.concatenate((test_result_data,tvf_test),axis=1)

train_result_data = np.concatenate((train_result_data,tvf_train_diff),axis=1)
valid_result_data = np.concatenate((valid_result_data,tvf_valid_diff),axis=1)
test_result_data = np.concatenate((test_result_data,tvf_test_diff),axis=1)
RMSE_am_tot, RMSE_am_low, RMSE_am_high,am_train,am_valid,am_test,am_train_diff,am_valid_diff,am_test_diff = RMSE_viscosity_bydomain(neuralmodel, ds, method='am',
                                                                                  boundary=7)
train_result_data = np.concatenate((train_result_data,am_train),axis=1)
valid_result_data = np.concatenate((valid_result_data,am_valid),axis=1)
test_result_data = np.concatenate((test_result_data,am_test),axis=1)

train_result_data = np.concatenate((train_result_data,am_train_diff),axis=1)
valid_result_data = np.concatenate((valid_result_data,am_valid_diff),axis=1)
test_result_data = np.concatenate((test_result_data,am_test_diff),axis=1)

RMSE_myega_tot, RMSE_myega_low, RMSE_myega_high,myega_train,myega_valid,myega_test,myega_train_diff,myega_valid_diff,myega_test_diff = RMSE_viscosity_bydomain(neuralmodel, ds,
                                                                                           method='myega', boundary=7)
train_result_data = np.concatenate((train_result_data,myega_train),axis=1)
valid_result_data = np.concatenate((valid_result_data,myega_valid),axis=1)
test_result_data = np.concatenate((test_result_data,myega_test),axis=1)

train_result_data = np.concatenate((train_result_data,myega_train_diff),axis=1)
valid_result_data = np.concatenate((valid_result_data,myega_valid_diff),axis=1)
test_result_data = np.concatenate((test_result_data,myega_test_diff),axis=1)

RMSE_compensation_tot, RMSE_compensation_low, RMSE_compensation_high ,compensation_train,compensation_valid,compensation_test,compensation_train_diff,compensation_valid_diff,compensation_test_diff= RMSE_viscosity_bydomain(
    neuralmodel, ds, method='visoscity_fuse', boundary=7)

train_result_data = np.concatenate((train_result_data,compensation_train),axis=1)
valid_result_data = np.concatenate((valid_result_data,compensation_valid),axis=1)
test_result_data = np.concatenate((test_result_data,compensation_test),axis=1)

train_result_data = np.concatenate((train_result_data,compensation_train_diff),axis=1)
valid_result_data = np.concatenate((valid_result_data,compensation_valid_diff),axis=1)
test_result_data = np.concatenate((test_result_data,compensation_test_diff),axis=1)

RMSE_compensation_wofix_tot, RMSE_compensation_wofix_low, RMSE_compensation_wofix_high,compensation_wofix_train,compensation_wofix_valid,compensation_wofix_test,compensation_wofix_train_diff,compensation_wofix_valid_diff,compensation_wofix_test_diff = RMSE_viscosity_bydomain(
    neuralmodel, ds, method='senond_grade_compensation', boundary=7)

train_result_data = np.concatenate((train_result_data,compensation_wofix_train),axis=1)
valid_result_data = np.concatenate((valid_result_data,compensation_wofix_valid),axis=1)
test_result_data = np.concatenate((test_result_data,compensation_wofix_test),axis=1)

train_result_data = np.concatenate((train_result_data,compensation_wofix_train_diff),axis=1)
valid_result_data = np.concatenate((valid_result_data,compensation_wofix_valid_diff),axis=1)
test_result_data = np.concatenate((test_result_data,compensation_wofix_test_diff),axis=1)

RMSE_compensation_error_tot, RMSE_compensation_error_low, RMSE_compensation_error_high = error_viscosity_bydomain(
    neuralmodel, ds, method='visoscity_fix_error_mean', boundary=7)

train_result_data = np.concatenate((train_result_data,RMSE_compensation_error_tot),axis=1)
valid_result_data = np.concatenate((valid_result_data,RMSE_compensation_error_low),axis=1)
test_result_data = np.concatenate((test_result_data,RMSE_compensation_error_high),axis=1)

tg_tot, tg_low, tg_high = error_viscosity_bydomain(
    neuralmodel, ds, method='tg', boundary=7)

train_result_data = np.concatenate((train_result_data,tg_tot),axis=1)
valid_result_data = np.concatenate((valid_result_data,tg_low),axis=1)
test_result_data = np.concatenate((test_result_data,tg_high),axis=1)

sctg_tot, sctg_low, sctg_high = error_viscosity_bydomain(
    neuralmodel, ds, method='sctg', boundary=7)

train_result_data = np.concatenate((train_result_data,tg_tot),axis=1)
valid_result_data = np.concatenate((valid_result_data,tg_low),axis=1)
test_result_data = np.concatenate((test_result_data,tg_high),axis=1)

ae_tot, ae_low, ae_high = error_viscosity_bydomain(
    neuralmodel, ds, method='ae', boundary=7)

train_result_data = np.concatenate((train_result_data,ae_tot),axis=1)
valid_result_data = np.concatenate((valid_result_data,ae_low),axis=1)
test_result_data = np.concatenate((test_result_data,ae_high),axis=1)

a_am_tot, a_am_low, a_am_high = error_viscosity_bydomain(
    neuralmodel, ds, method='a_am', boundary=7)

train_result_data = np.concatenate((train_result_data,a_am_tot),axis=1)
valid_result_data = np.concatenate((valid_result_data,a_am_low),axis=1)
test_result_data = np.concatenate((test_result_data,a_am_high),axis=1)

a_cg_tot, a_cg_low, a_cg_high = error_viscosity_bydomain(
    neuralmodel, ds, method='a_cg', boundary=7)

train_result_data = np.concatenate((train_result_data,a_cg_tot),axis=1)
valid_result_data = np.concatenate((valid_result_data,a_cg_low),axis=1)
test_result_data = np.concatenate((test_result_data,a_cg_high),axis=1)

a_tvf_tot, a_tvf_low, a_tvf_high = error_viscosity_bydomain(
    neuralmodel, ds, method='a_tvf', boundary=7)

train_result_data = np.concatenate((train_result_data,a_tvf_tot),axis=1)
valid_result_data = np.concatenate((valid_result_data,a_tvf_low),axis=1)
test_result_data = np.concatenate((test_result_data,a_tvf_high),axis=1)

to_cg_tot, to_cg_low, to_cg_high = error_viscosity_bydomain(
    neuralmodel, ds, method='to_cg', boundary=7)

train_result_data = np.concatenate((train_result_data,to_cg_tot),axis=1)
valid_result_data = np.concatenate((valid_result_data,to_cg_low),axis=1)
test_result_data = np.concatenate((test_result_data,to_cg_high),axis=1)

c_cg_tot, c_cg_low, c_cg_high = error_viscosity_bydomain(
    neuralmodel, ds, method='c_cg', boundary=7)

train_result_data = np.concatenate((train_result_data,c_cg_tot),axis=1)
valid_result_data = np.concatenate((valid_result_data,c_cg_low),axis=1)
test_result_data = np.concatenate((test_result_data,c_cg_high),axis=1)

c_tvf_tot, c_tvf_low, c_tvf_high = error_viscosity_bydomain(
    neuralmodel, ds, method='c_tvf', boundary=7)

train_result_data = np.concatenate((train_result_data,c_tvf_tot),axis=1)
valid_result_data = np.concatenate((valid_result_data,c_tvf_low),axis=1)
test_result_data = np.concatenate((test_result_data,c_tvf_high),axis=1)

b_cg_tot, b_cg_low, b_cg_high = error_viscosity_bydomain(
    neuralmodel, ds, method='b_cg', boundary=7)

train_result_data = np.concatenate((train_result_data,b_cg_tot),axis=1)
valid_result_data = np.concatenate((valid_result_data,b_cg_low),axis=1)
test_result_data = np.concatenate((test_result_data,b_cg_high),axis=1)



fragility_tot, fragility_low, fragility_high = error_viscosity_bydomain(
    neuralmodel, ds, method='fragility', boundary=7)

train_result_data = np.concatenate((train_result_data,fragility_tot),axis=1)
valid_result_data = np.concatenate((valid_result_data,fragility_low),axis=1)
test_result_data = np.concatenate((test_result_data,fragility_high),axis=1)


b_tvf_tot, b_tvf_low, b_tvf_high = error_viscosity_bydomain(
    neuralmodel, ds, method='b_tvf', boundary=7)

train_result_data = np.concatenate((train_result_data,b_tvf_tot),axis=1)
valid_result_data = np.concatenate((valid_result_data,b_tvf_low),axis=1)
test_result_data = np.concatenate((test_result_data,b_tvf_high),axis=1)

be_tot, be_low, be_high = error_viscosity_bydomain(
    neuralmodel, ds, method='be', boundary=7)

train_result_data = np.concatenate((train_result_data,be_tot),axis=1)
valid_result_data = np.concatenate((valid_result_data,be_low),axis=1)
test_result_data = np.concatenate((test_result_data,be_high),axis=1)

sc_t_tot, sc_t_low, sc_t_high = error_viscosity_bydomain(
    neuralmodel, ds, method='sc_t', boundary=7)

train_result_data = np.concatenate((train_result_data,sc_t_tot),axis=1)
valid_result_data = np.concatenate((valid_result_data,sc_t_low),axis=1)
test_result_data = np.concatenate((test_result_data,sc_t_high),axis=1)

# att1_tot, att1_low, att1_high = att_viscosity_bydomain(
#     neuralmodel, ds, method='self_attention1', boundary=7)
#
# train_result_data = np.concatenate((train_result_data,att1_tot),axis=1)
# valid_result_data = np.concatenate((valid_result_data,att1_low),axis=1)
# test_result_data = np.concatenate((test_result_data,att1_high),axis=1)
#
# att2_tot, att2_low, att2_high = att_viscosity_bydomain(
#     neuralmodel, ds, method='self_attention2', boundary=7)
#
# train_result_data = np.concatenate((train_result_data,att2_tot),axis=1)
# valid_result_data = np.concatenate((valid_result_data,att2_low),axis=1)
# test_result_data = np.concatenate((test_result_data,att2_high),axis=1)
#
# att3_tot, att3_low, att3_high = att_viscosity_bydomain(
#     neuralmodel, ds, method='self_attention3', boundary=7)
#
# train_result_data = np.concatenate((train_result_data,att3_tot),axis=1)
# valid_result_data = np.concatenate((valid_result_data,att3_low),axis=1)
# test_result_data = np.concatenate((test_result_data,att3_high),axis=1)
#
# att4_tot, att4_low, att4_high = att_viscosity_bydomain(
#     neuralmodel, ds, method='self_attention4', boundary=7)
#
# train_result_data = np.concatenate((train_result_data,att4_tot),axis=1)
# valid_result_data = np.concatenate((valid_result_data,att4_low),axis=1)
# test_result_data = np.concatenate((test_result_data,att4_high),axis=1)
#
# att5_tot, att5_low, att5_high = att_viscosity_bydomain(
#     neuralmodel, ds, method='self_attention5', boundary=7)
#
# train_result_data = np.concatenate((train_result_data,att5_tot),axis=1)
# valid_result_data = np.concatenate((valid_result_data,att5_low),axis=1)
# test_result_data = np.concatenate((test_result_data,att5_high),axis=1)
#
time3 = time.time()
print("train:"+str(time2-time1))
print("prediction:"+str(time3-time2))

np.savetxt("./each_point/Deepmelt-new/Deepmelt_avg_train_ne_"+str(seed-162)+".csv",train_result_data,delimiter=",")
np.savetxt("./each_point/Deepmelt-new/Deepmelt_avg_valid_ne_"+str(seed-162)+".csv",valid_result_data,delimiter=",")
np.savetxt("./each_point/Deepmelt-new/Deepmelt_avg_test_ne_"+str(seed-162)+".csv",test_result_data,delimiter=",")

# resultn_visoscity_fuse = error_viscosity_bydomain(neuralmodel, ds, method='visoscity_fuse',boundary=7)
# aaa = 1
#
# np.savetxt("./each_point/Deepmelt/Deepmelt_visoscity_avg_wiec_fuse_point_error_DR=0.05_NE_"+str(seed-162)+".csv",resultn_visoscity_fuse.numpy().reshape(-1, 1),delimiter=",")
