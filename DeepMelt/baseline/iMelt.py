...
class model(torch.nn.Module):
    ...
    #
    #     return self.b_cg_o(x) + torch.tanh(c11) * c12
    ...
    #
    #     return self.b_tvf_o(x) + torch.tanh(c11) * c12
    ...
    #     c12 = self.scale_compensation2_output(c12)
    #
    #     return self.be_o(x) + torch.tanh(c11) * c12
    ...
    #
    #     c12 = self.sc_t(x, T)
    #     for layer in self.scale_compensation4:  # Feedforward
    #         # c2 = layer(c2)
    #         c12 = self.dropout(self.activation_function(layer(c12)))
    #     c12 = self.scale_compensation2_output(c12)
    #
    #     return self.sc_t(x, T) + torch.tanh(c11) * c12
    ...
    def senond_grade_compensation(self, x, T):
        ...
        c21 = self.machine_compensation2_output(c21)
...
def training(neuralmodel, ds, criterion, optimizer, save_switch=True, save_name="./temp", train_patience=50,
    ...
    while val_ex <= train_patience:
        ...
        if mode == "pretrain":
            ...
            loss = loss_ag + loss_myega + loss_am + loss_cg + loss_tvf   # +  loss_compensation
        ...
        with torch.set_grad_enabled(False):
            ...
            if mode == "pretrain":
                ...
                loss_v = loss_ag_v + loss_myega_v + loss_am_v + loss_cg_v + loss_tvf_v  #+ loss_compensation_v
    ...
    if verbose == True:
        ...
        print(
            'Scaled valid loss values are {:.2f}, {:.2f}, {:.2f} for Tg, entropy, viscosity (AG)'.format(
                loss_tg_v, loss_entro_v, loss_ag_v
            ))
...
aaa = 1
np.savetxt("./each_point/imelt/imelt_ag_point_error_"+str(seed-162)+".csv",resultn_ag.numpy().reshape(-1, 1),delimiter=",")
np.savetxt("./each_point/imelt/imelt_cg_point_error_"+str(seed-162)+".csv",resultn_cg.numpy().reshape(-1, 1),delimiter=",")
np.savetxt("./each_point/imelt/imelt_tvf_point_error_"+str(seed-162)+".csv",resultn_tvf.numpy().reshape(-1, 1),delimiter=",")
np.savetxt("./each_point/imelt/imelt_am_point_error_"+str(seed-162)+".csv",resultn_am.numpy().reshape(-1, 1),delimiter=",")
np.savetxt("./each_point/imelt/imelt_myega_point_error_"+str(seed-162)+".csv",resultn_myega.numpy().reshape(-1, 1),delimiter=",")
