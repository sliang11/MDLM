...
for n_estimators in [100,500,1000]:
    ...
    regr_rf.fit(train_melt_X,train_melt_Y)

    # svm.fit(X_train, y_train)
    score = regr_rf.score(valid_melt_X,valid_melt_Y)
    if score > best_score:
        best_score = score
        best_parameters = {'n_estimators':n_estimators}
time1 = time.time()
xgb = RandomForestRegressor(**best_parameters) #使用最佳参数，构建新的模型
...
# resultn=mean_squared_error(test_melt_Y, predicty3,multioutput = "raw_values" )
time3 = time.time()
...
# random_forest = resultn.T
# random_forest.(".\\each_point\\rf_point_error_0.csv",index=False,header=False)
np.savetxt("./each_point/rf/rf_point_error_"+str(seed-162)+".csv",resultn,delimiter=",")
# xgb = RandomForestRegressor(**best_parameters) #使用最佳参数，构建新的模型
